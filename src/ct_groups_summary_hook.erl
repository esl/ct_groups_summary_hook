...
init(_Id, Opts) ->
    {ok, #{
        logs => proplists:get_value(logs, Opts, false),
        total_ok => 0,
        total_failed => 0,
...
do_update_group_status(
    ...
    NewGroupStatus = #group_status{
        status = Status,
        n_failed = length(Failed),
        failed = Failed
    },
    NewSuiteState =
        case maps:get(GroupNameWPath, SuiteState, undefined) of
            ...
            PrevGroupStatus ->
                SuiteState#{
                    GroupNameWPath := merge_group_status(
                        PrevGroupStatus,
                        NewGroupStatus
                    )
                }
...
write_groups_summary(
    ...
    case PrivDir of
        ...
        _ ->
            ...
            ok = file:write_file(
                filename:join([SuiteDir, "groups.summary"]),
                io_lib:format(
                    "~p.\n"
                    "~p.\n"
                    "~p.\n",
...
acc_groups_summary(
    _GroupName,
    #group_status{status = ok, n_failed = NFailedTests},
...
acc_groups_summary(
    _GroupName,
    #group_status{status = failed, n_failed = _NFailedTests},
    {OkGroupsAcc, FailedGroupsAcc, EventuallyOkAcc}
) ->
